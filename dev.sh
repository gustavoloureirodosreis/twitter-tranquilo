...
function dkbuild {
    CD=$(pwd)
    cd $PROJ_BASE
    docker build -t twitter-tranquilo .
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development twitter-tranquilo npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./twitter-tranquilo/
  ssh ubuntu@$HOST_PROD "
    cd ~/twitter-tranquilo
    source dev.sh
    dkbuild
    dkrun_prod
  "
...
function dk {
    docker exec -it twitter-tranquilo $@
}
...
echo_green "Welcome to twitter-tranquilo's dev env"
